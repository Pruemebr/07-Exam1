...
Authors: David Mutchler, Dave Fisher, Matt Boutell, Amanda Stouder,
         their colleagues and Bryce Pruemer.  March 2018.
...
def problem2(line1, line2, thickness, win):
    ...
    #2
    x1val = (line1.start.x + line1.end.x)/2
    y1val = (line1.start.y + line1.end.y)/2

    x2val = (line2.start.x + line2.end.x) / 2
    y2val = (line2.start.y + line2.end.y) / 2

    midline1 = rg.Point(x1val, y1val)
    midline2 = rg.Point(x2val, y2val)
    rect = rg.Rectangle(midline1, midline2)
    rect.attach_to(win)
    win.render()
